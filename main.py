...
def extract_estrato(texto):
    ...
    return match.group() if match else "Estrato socioeconómico no encontrado"
...
def chatbot(query: str = Query(..., description="Consulta relacionada con el estrato socioeconómico del infractor:")):
 ...
 # Filtrar los resultados según los sinónimos   
    results = [item for item in datos_list if any(s in item['ssb'].lower() for s in synonyms)]
    
 # Extraer el número del estrato socioeconómico ingresado por el usuario o usar el primer valor encontrado o dejar vacio
    ssb_value = results[0]['ssb'] if results else extract_estrato(query)
